...
sed -i -e "s:/var/packages/gazebo/ubuntu:${FAKE_HOME}/pbuilder/${DISTRO}_result/:g" ${SCRIPT_DIR}/lib/docker/debbuild-base.bash

echo "3. Calling jenkins script"
# pbuilder via sudo needs to own home
chmod +x ${SCRIPT_DIR}/docker/multidistribution-debbuild.bash
# root needs to own home for pbuilder
sudo chown -R root:root ${FAKE_HOME}
HOME=${FAKE_HOME} ${SCRIPT_DIR}/docker/multidistribution-debbuild.bash
